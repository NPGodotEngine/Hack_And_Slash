...
# Damage multiplier
##
# For scaling damage from weapon
...
# The hit damage from weapon
var _weapon_hit_damage: HitDamage = null
...
func  get_scaled_hit_damage() -> HitDamage:
	if _weapon_hit_damage:
		return HitDamage.new().init(
			_weapon_hit_damage._attacker,
			_weapon_hit_damage._weapon,
			_weapon_hit_damage._damage * _damage_multiplier,
			false,
			0.0,
			Color.white
		)
	return HitDamage.new().get_default()
